...
class InvertedResidualBlock(nn.Module):
    def __init__(self, inp, oup, expand_ratio=4):
        ...
        self.bottleneckBlock = nn.Sequential(
            ...
            ACBlock(hidden_dim, hidden_dim, 3, groups=hidden_dim),
            # nn.BatchNorm2d(hidden_dim),
            nn.ReLU6(inplace=True),
            # pw-linear
...
class GD_Layer(nn.Module):
    """ A GDFSFM layer for one stage.
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.lam = LAM_Module(in_dim=dim,
                              depth=depth,)
        self.detail_feature = InvertedResidualBlock(inp=dim,
                                                    oup=dim)
    ...
    def forward(self, x):
        ...
        detail = x.view(B, H, W, C).contiguous()
        # b c h w
        detail = detail.permute(0, 3, 1, 2)
        detail = self.detail_feature(detail)
        # b h w c
        detail = detail.permute(0, 2, 3, 1)
        #
        detail = detail.view(B, -1, C).contiguous()
        ...
        if self.downsample is not None:
            x = self.downsample(out_fuse)
        return x


class GD_Layer_up(nn.Module):
    """ Upsample
...
class UTransCNN(nn.Module):
    def __init__(self, opt):
        super(UTransCNN, self).__init__()
        ...
        for i_layer in range(self.num_layers):
            layer = GD_Layer(dim=int(self.embed_dim * 2 ** i_layer),
        ...
        for i_layer in range(self.num_layers):
            ...
            if i_layer == 0:
                ...
                layer_up = GD_Layer_up(dim=int(self.embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                         input_resolution=(
